...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train = x_train.reshape(num_train,-1) # reshape하는 의미가 뭐지?
...
pp = torch.rand(3,5)
pp.reshape(3,-1) # -1이라는 건 앞에서 차원맞추고 남는거에 맞추겠다는 말
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    values,indices = torch.topk(dists[:,i],k,largest=False) # i열에서 가장 작은 것부터 k개뽑기
    values,indices = y_train[indices].unique(return_counts=True) # y_train에서 같은 인덱스인거 뽑고 unique한 것만
    y_pred[i] = values[indices.argmax()] # 인덱스중에 가장 큰거
  # topk = top k개
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
class KnnClassifier:
  ...
  def predict(self, x_test, k=1):
    ...
    y_test_pred = predict_labels(dists, self.y_train,k=k) # def에서 받아온 k사용
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_test_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_folds = torch.chunk(x_train, num_folds) # num_folds의 개수만큼 쪼갬
  ...
  # Replace "pass" statement with your code
  for k in k_choices: 
    k_to_accuracies[k] = [] # k자리에 넣을 accuracy
    for i in range(num_folds): 
      fx_test = x_train_folds[i] # i번째 뭉텅이를 test로 사용
      fy_test = y_train_folds[i] # ""
      fx_train = torch.cat([x_train_folds[j] for j in range(num_folds) if i !=j]) # i(test데이터)번째 뭉텅이가 아닌 애들 행(아래쪽)으로 합치기
      fy_train = torch.cat([y_train_folds[j] for j in range(num_folds) if i !=j]) # ""
      classifier = KnnClassifier(fx_train, fy_train) # knnclassifier 실행
      acc = classifier.check_accuracy(fx_test, fy_test, k=k, quiet=True) # 정확도 저장
      k_to_accuracies[k].append(acc) # k_to_caauracies딕셔너리의 k번째 key에 value(리스트 형태)로 추가
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  # Replace "pass" statement with your code
  means = []
  for k, accs in sorted(k_to_accuracies.items()): # items는 key와 value모두 있는 것 의미함.sorted는 key별로 정렬?하고 key별로 뽑아냄?
    means.append(statistics.mean(accs)) # 같은 key를 가진 여러 i들(accs)의 평균을 k별로 구함. 
  best_k = sorted(k_to_accuracies.items())[(torch.argmax(torch.tensor(means))).item()][0] # 평균리스트 중에 가장 큰 값의 인덱스를 받아오고, 최댓값을 가진 곳의 index 알아내는 것에서 item() 쓰면 값만 뽑아냄
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
