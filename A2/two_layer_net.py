...
def nn_forward_pass(params, X):
    ...
    dtype = X.dtype
    hidden = torch.max(torch.FloatTensor([0]).to(
        dtype).cuda(), (X.mm(W1)+b1.reshape(1, -1)).to(dtype)) # 이해 안됨..
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    scores -= maximum_score # 모든 element에 대해 연산 적용 - 빠른 컴퓨팅 연산을 위해 빼기사용

    out = torch.exp(scores)
    scores_sums = torch.sum(out,1).reshape(N,1)
    out /= scores_sums
    index_right_cls = (range(N),y) # range를 넣음으로써 한 행씩 돌아가면서 y열에 있는 element 뽑음
    correct_scores = out[index_right_cls] # out[y]를 하면 y번째 행 자체를 뽑아서 행렬 만듬
    loss = -1 * torch.sum(torch.log(correct_scores)) # cross-entropy loss 식 적용 (log 씌우고 -붙이기)
    loss /=N # 평균 loss
    loss += reg * (torch.sum(W1 **2) + torch.sum(W2 **2) # L2 regularization
    ...
    # Replace "pass" statement with your code
    d1_dout = out.clone()
    d1_dout[range(N),y] -= 1 # 왜 1을 빼는거지?

    dW2 = h1.t().mm(d1_dout)
    db2 = torch.sum(d1_dout,0) # 클래스별로

    dh1 = d1_dout.mm(W2.t())
    dh1[h1 == 0] = 0 # 왜?

    dW1 = X.t().mm(dh1)
    db1 = torch.sum(dh1,0) # 클래스별로
...
def nn_predict(params, loss_func, X):
  ...
  h = torch.max(z,torch.tensor([0.0]).cuda()) # max(0,값) # .cuda()는 왜 하는 거야?
  out = h.mm(params['W2']) + params['b2']
  y_pred = torch.argmax(out,1) # 데이터별로 가장 큰 값을 가지는 클래스 
  ###########################################################################
  #                              END OF YOUR CODE                           #
  ###########################################################################
