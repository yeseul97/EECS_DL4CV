...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    # x.shape[0]는 배치사이즈
    out = x.reshape(x.shape[0], -1).mm(w) + b # 각 x의 데이터에 w를 곱해서 사용
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b)
  ...
  def backward(dout, cache):
    ...
    dw = x.reshape(x.shape[0], -1).t().mm(dout) # reshape은 어떻게 알고 할까..?
    db = torch.sum(dout,0) # b에 대해 미분하면 dout만 남음
...
class ReLU(object):
  ...
  def forward(x):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    out = torch.max(torch.tensor([0], dtype=x.dtype, device=x.device),x) # 식: max(0,x)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = x
  ...
  def backward(dout, cache):
    ...
    Input:
    - dout: Upstream derivatives, of any shape # dout은 어디서 나온거야??
    ...
    # 조건문은 TRUE, FALSE로 반환함 -> dout값들 중에 0보다 큰 값만 살아남음
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    # two-layer net이면 이렇게 층을 구성해야 하는거야??
    ...
    loss = soft_loss + self.reg * (torch.sum(self.params['W1'] * self.params['W1']) +
    ...
    grads['W2'] = dw2 + 2*self.params['W2'] * torch.tensor([self.reg], device=dw2.device, dtype = dw2.dtype)
    grads['b2'] = db2
    grads['W1'] = dw + 2 * self.params['W1'] * torch.tensor([self.reg], device=dw2.device, dtype = dw2.dtype)
    grads['b1'] = db
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(self.num_layers):
        self.params[f'W{i+1}'] = weight_scale * torch.randn(dims[i], dims[i+1]).to(device).to(dtype)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers): 
        if i != self.num_layers-1:
            scores, cache = Linear_ReLU.forward(scores, self.params[f'W{i+1}'],
                                                    self.params[f'b{i+1}'])
        else: # 마지막층은 Linear
            scores, cache = Linear.forward(scores, self.params[f'W{i+1}'],
                                           self.params[f'b{i+1}'])
        ...
        L2reg += torch.sum(self.params[f'W{i+1}'])**2 # 매 층마다 L2_regularization
    ...
    for i in range(self.num_layers-1, -1, -1): # -1까지는 0까지 하면 됨.
        if i != self.num_layers-1:
            if self.use_dropout:
...
def sgd_momentum(w, dw, config=None):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  # SGD_momentum 수식 : V(t) = m * V(t-1) - leanring_rate * dw 
...
def rmsprop(w, dw, config=None):
    # 기울기의 단순한 누적만으로 충분치 않음 
    # -> RMSProp은 지수 가중 이동 평균을 사용하여 최신 기울기들이 더 크게 반영되도록 함!!
  """
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      # 일정 비율을 0으로 만드는 건 이해되는데, (1-p)로 나눈 값을 곱하는 이유는 뭘까??
      mask = (torch.randn(*x.shape) < (1-p)).to(x.dtype).to(device) / (1-p)
      out = x * mask
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
